			...
			<div class="modal-dialog modal-dialog-centered" role="document">
				<div class="modal-content">
					...
					<div class="modal-footer">
						...
						<button type="button" class="btn btn-primary" data-bs-dismiss="modal" onclick="enviarAprovRDO(<?php echo $data9->IDMOV; ?>)" id="btncancelar">Sim, enviar!</button>
					</div>
				</div>
			</div>
		</div>
				...
				<div class="col-xl-10 col-lg-10 col-xxl-10 col-md-10">
					...
					<div class="btn-group mb-1">
						...
						<?php if ($data9->STATUS == 'B'): ?>
							<button type="button" class="btn btn-primary light px-3 testebotao" id="botaordosalvar" class="nav-link" onclick="updateRDO2(<?= $IDMOV; ?>)">
								<i class="fa fa-floppy-o" aria-hidden="true"></i> Salvar Alterações
							</button>
						<?php else: ?>
							<button type="button" id="botaordosalvar3" class="btn btn-primary light px-3 testebotao" class="nav-link" onclick="updateRDO(<?= $IDMOV; ?>)">
								<i class="fa fa-floppy-o" aria-hidden="true"></i> Salvar Alterações
							</button>
						<?php endif; ?>
		...
		<div id="collapsehd4" class="collapse show" aria-labelledby="hd4" data-bs-parent="#accordion-one">
			<div class="accordion-body-text">
				...
				<div class="mb-3">
                    ...
                    <button id="btncomentario" type="button" class="btn btn-primary btn-sm mt-1" onclick="enviarcomentario(<?= $IDMOV; ?>,5)"> Enviar </button>         
                </div>
...
<script>
    function updateRDO(IDMOV) {
        ...
        if (SAMF_VS_MONTAGEM_VALID == 0 && SAMF_VS_MOBILIZACAO_VALID == 0 && SAMF_VS_DESMONTAGEM_VALID == 0 && SAMF_VS_ADEQUACAO_VALID == 0 && SAMF_VS_MANUTENCAO_VALID == 0) {
            toastr.warning( "Preencha ao menos uma informação!","ATIVIDADES", {
                positionClass: "toast-top-right",
                timeOut: 5000,
                closeButton: true,
                debug: false,
        ...
        rowsreferencia.forEach(row => {
            ...
            if (ATIVIDADE_REF == 1) {
                ...
                // toastr.warning("Informação não preenchida!", "ATIVIDADE REFERÊNCIA", {
                //     positionClass: "toast-top-right",
                //     timeOut: 5000,
                //     closeButton: true,
                //     debug: false,
	...
	$(document).ready(function() {
		...
		$('input[name="optradio"]').on('change', function() {
			if ($('#horaextrasim').is(':checked')) {
				$('#horaextrasimaccordion').slideDown(); // Mostrar com efeito suave
			} else {
				$('#horaextrasimaccordion').slideUp(); // Ocultar com efeito suave
			}
		});
    ...
    function calculateWorkingTime() {
        ...
        $('#dynamicTableFuncionariosGeral tbody tr').each(function() {
            ...
            if (entradaInput.value && saidaInput.value) {
                ...
                // Atualiza o campo QUANTIDADENORMAL
                quantidadeNormalField.value = quantidadeNormal - 60; // Ajuste de horas de almoço
            }
        });
    }
